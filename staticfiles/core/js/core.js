(function () {
    ...
    (function initMobileHeaderHide() {
        ...
        let ticking = false;
        const threshold = 8; // minimal delta to trigger

        const onScroll = () => {
            if (window.innerWidth >= 1024) {
                // Desktop: don't toggle
        ...
        // Ensure header is visible when resizing to desktop
    ...
    // Amenity chip behavior: convert checkbox list into clickable chips
    (function initAmenityChips() {
        const containers = document.querySelectorAll('.amenity-row');
        if (!containers.length) return;

        containers.forEach((container) => {
            // Find all checkbox inputs and their labels
            const inputs = Array.from(container.querySelectorAll('input[type="checkbox"]'));
            inputs.forEach((input) => {
                const label = input.closest('label') || container.querySelector(`label[for="${input.id}"]`);
                if (!label) return;
                // decorate label
                label.classList.add('amenity-chip');
                // add an icon span if one doesn't exist
                if (!label.querySelector('.chip-icon')) {
                    const icon = document.createElement('span');
                    icon.className = 'chip-icon';
                    // simple default icon (checkbox square)
                ...
                // click on label toggles the checkbox (native behavior); ensure visual sync
                label.addEventListener('click', (ev) => {
                    // allow native toggle, then sync after a tiny delay to pick up input.checked
                ...
                // keep in sync if input changed by other scripts
    ...
    const initPropertyMaps = () => document.querySelectorAll('[data-property-map]').forEach((node) => {
        ...
        // Enable scroll-to-zoom only when the user's cursor is over the map
        node.addEventListener('mouseenter', () => {
            try { map.scrollWheelZoom.enable(); } catch (e) { /* noop */ }
        });
        node.addEventListener('mouseleave', () => {
            try { map.scrollWheelZoom.disable(); } catch (e) { /* noop */ }
        });
        // Use CARTO basemaps to avoid volunteer-run tile server blocking and allow higher zoom levels
